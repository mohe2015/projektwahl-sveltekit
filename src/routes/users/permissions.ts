...
export const validator: Validator<Partial<New<RawUserType>>, { [key: string]: string }> = (
	...
	return andThen(assertObjectType(value), (value) => {
		...
		const type = assertOptionalPropertyOf(
			value,
			'type',
			assertEnumProperty(['admin', 'helper', 'voter'])
		);
		...
		const project_leader_id = assertOptionalPropertyOf(
			value,
			'project_leader_id',
			assertNumberProperty
		);
		const force_in_project_id = assertOptionalPropertyOf(
			value,
			'force_in_project_id',
			assertNumberProperty
		);
